 ...
 *
 * Maintains a scroll buffer for each tab, continuously collecting console events.
 * Buffer is automatically cleared when tab navigates to a new domain, avoiding log mixing between different sites.
 */
...
/**
 * Extract safe preview data from CDP RemoteObject, discard objectId to avoid memory leaks
 */
...
class ConsoleBuffer {
  ...
  /**
   * Check whether specified tab is in buffer mode capture
   */
  ...
  /**
   * Clear buffer for specified tab
   */
  clear(
    tabId: number,
  ...
  /**
   * Read buffer content of specified tab
   */
  ...
  private handleDebuggerEvent(
    ...
    if (method === 'Runtime.consoleAPICalled' && p) {
      ...
      state.messages.push({
        ...
        // Only store safe preview data, avoid memory leaks
