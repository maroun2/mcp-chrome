<template>
  <div class="popup-container agent-theme" :data-agent-theme="agentTheme">
          ...
          <div class="entry-card">
            ...
            <button class="entry-item" @click="currentView = 'local-model'">
              ...
              <div class="entry-content">
                ...
                <span class="entry-desc">Semantic Engine & Models</span>
              </div>
              <svg
                class="entry-arrow"
                viewBox="0 0 24 24"
    ...
    <!-- Side panel handles workflow management; editor opens in separate window -->
...
// AgentChat theme - loaded from preload to stay in sync with sidepanel
...
// Flow editor opens in separate window; popup no longer shows the full list
...
// Run history and overrides are viewed in the sidepanel page
const startRecording = async () => {
  // TODO: Recording & replay coming soon, intercept for now
    ...
    try {
      ...
      if (result && result.success === false) {
        ...
        if (failed && failed.stepId) {
          // Open standalone editor window and focus the failed node
...
// Legacy "clone/publish/schedule/overrides" are handled in sidepanel or editor
...
function openWorkflowSidepanel() {
  // TODO: Workflow feature coming soon, intercept for now
...
async function toggleWebEditor() {
  try {
    ...
    console.warn('Failed to toggle web edit mode:', error);
  }
}
...
async function toggleElementMarker() {
  try {
    ...
    if (!tab?.id) {
      console.warn('Failed to get current tab');
      return;
    }

    // Send message to background, start element annotation
