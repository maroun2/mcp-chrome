...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Not recorded in history to avoid flooding; history is recorded on user actions (connect/add/delete etc.).
  }
  ...
  function onConnect(sourceId: string, targetId: string, label: string = 'default') {
    ...
    // Single outgoing edge per label: remove existing edge from same source with same label
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
  ...
  // Fallback layout: layered + barycenter ordering (no external library dependency)
  ...
  // - Dynamically imports elkjs to avoid permanent bundle size increase
  // - Falls back to layoutFallback() on failure
