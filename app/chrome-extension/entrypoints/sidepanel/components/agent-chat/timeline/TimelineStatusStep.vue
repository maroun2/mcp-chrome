...
// Whether currently in running state
...
// Timeout ID for scheduled text updates
let timeoutId: ReturnType<typeof setTimeout> | null = null;

// Track previous running state to detect state changes
let wasRunning = false;

// Start timer
function startInterval(): void {
  if (timeoutId) return;
  // Update text at random 5-8 second intervals
  const scheduleNext = () => {
...
// Watch running state changes - only process when state actually changes
watch(isRunning, (running) => {
  // Only regenerate text and start timer when transitioning from not-running to running
