...
// Shadow type effect (Drop Shadow / Inner Shadow)
...
// Blur type effect (Layer Blur / Backdrop Blur)
...
// Raw effect that cannot be parsed
...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Effect items cache per element (current editing session only)
  // Reason for using WeakMap:
  // 1. Hidden effects do not write CSS (enabled=false) but need to be remembered within the session for resume
  // 2. WeakMap ensures memory is automatically freed when the element is removed, no manual cleanup needed
  // 3. Only reads inline style (not computed), so the cache is only used to retain hidden operations by the user
  ...
  if (headerActionsContainer) {
    // Place the + button to the right of the group header (left of chevron)
    headerActionsContainer.insertBefore(addBtn, headerActionsContainer.firstChild);
    disposer.add(() => addBtn.remove());
  } else {
    // Fallback: show toolbar in content area
  ...
  function isEditing(): boolean {
    // Only block refresh when a popover is open or a transaction is in progress
    // Avoid overly broad focus detection that prevents external style changes from syncing
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after close to ensure currentItems matches actual inline
    // Avoid inconsistencies caused by browser normalization/correction of values
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Use document capture-phase listener to ensure closing works outside the Effects control
  ...
  // Listen for capture-phase click events on document
  ...
  // Listen for capture-phase keyboard events on root
