...
function createDisplayIcon(value: DisplayValue): SVGElement {
  ...
  // Container border (dashed rectangle represents container)
  ...
  switch (value) {
    ...
    case 'inline':
      ...
      break;
    case 'inline-block':
      // One block on the left edge, two text lines on the right
      addBlock(3.5, 4.5, 3.5, 6);
      addLine(8, 5.5, 4);
      addLine(8, 8.5, 3);
      break;
    case 'flex':
      // Three horizontally arranged flex children
...
function createHorizontalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // Content block X position varies by alignment
  ...
  // Two small squares represent children (horizontal arrangement becomes vertical)
...
function createVerticalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // Content block Y position varies by alignment
  ...
  // Two small squares represent children
  const block1 = document.createElementNS(SVG_NS, 'rect');
